...
class OneSecondDisplayAggregator:
    """
    新的显示聚合器：所有模式都监听2秒钟，根据不同模式采用不同的稳定策略
    """
    def __init__(self, monitoring_duration=2.0):
    ...
    def _process_overlap_mode(self):
        """overlap模式：找符合6-12范围的、最稳定的值"""
        if not self.collected_data['size_values']:
            print("Overlap模式: 无尺寸数据")
            return

        sizes = [v for t, v in self.collected_data['size_values']]
        print(f"Overlap模式原始数据: {[f'{s:.2f}' for s in sizes]}")

        # 筛选6-12范围内的有效值
        valid_sizes = [s for s in sizes if 6 <= s <= 12]

        # 如果筛选后没有值，则退回到使用所有原始值
        target_sizes = valid_sizes if valid_sizes else sizes

        if not target_sizes:
            print("Overlap模式: 无有效尺寸数据")
            return

        # --- 核心修正：寻找最稳定的值，而不是最小值 ---
        # 策略：通过将尺寸四舍五入到一位小数来进行分组，然后找到最常见的组。
        if not target_sizes:
            return

        # 将尺寸取整到一位小数用于分箱计数
        binned_sizes = [round(s, 1) for s in target_sizes]
        if not binned_sizes:
            return
            
        # 统计每个分箱的频率
        size_counts = Counter(binned_sizes)
        # 找到最常见的分箱值
        most_common_bin = size_counts.most_common(1)[0][0]

        # 筛选出所有属于最常见分箱的原始尺寸值
        stable_values = [s for s in target_sizes if round(s, 1) == most_common_bin]

        # 对这些最稳定的值求平均，以获得更精确的结果
        if stable_values:
            final_size = sum(stable_values) / len(stable_values)
            self.stable_result['size_cm'] = final_size
            print(f"Overlap模式: 最稳定值(基于 {len(stable_values)} 个样本) {final_size:.2f}cm")
        else:
            # 极端情况下的后备方案，虽然不太可能发生
            self.stable_result['size_cm'] = target_sizes[0]
            print(f"Overlap模式: [后备] 取第一个值 {target_sizes[0]:.2f}cm")
    ...
    def _process_digit_mode(self):
        ...
        # ==================== 核心修正：优先匹配用户输入 ====================
        # 检查在2秒的监听期内，用户指定的目标数字是否出现过
        if self.target_digit is not None and self.target_digit in digit_sizes:
            # 如果找到了，就用所有出现过的该数字的尺寸计算平均值
            target_sizes = digit_sizes[self.target_digit]
            avg_size = sum(target_sizes) / len(target_sizes)
            self.stable_result['size_cm'] = avg_size
            print(f"Digit模式: [优先匹配] 成功定位目标 {self.target_digit}, 尺寸 {avg_size:.2f}cm")
            # 匹配成功，直接返回，不再执行后续的复杂推断逻辑
            return
        # ======================== 修正结束 =========================
        ...
        print(f"Digit模式分析(后备方案): 数字频率 {dict(digit_counts)}, 目标数字 {self.target_digit}")
...
class MeasurementApp:
    def __init__(self, root_window):
        ...
        try:
            ...
            messagebox.showerror("初始化错误", f"系统组件初始化失败: {e}")
            self.root.destroy()
            return
    ...
    def create_widgets(self):
        ...
        self.serial_status_label.grid(row=len(mode_buttons)+3, column=0, sticky='ew', padx=5, pady=5)
    ...
    def _start_measurement_for_mode(self, mode):
        ...
        if mode == 'digit':
            ...
            if selected_digit is not None:
                ...
                self.status_var.set(f"正在测量数字 {selected_digit}... (2秒)")
            else:
            ...
            self.status_var.set(f"正在测量 {mode_names.get(mode, mode)} 模式... (2秒)")
    ...
    def _refresh_all(self):
        ...
        self.stabilizer.reset_all() # 假设stabilizer有reset_all方法
    ...
    def _reset_mode_state(self):
        """重置状态"""
        # 这个方法现在可以更简单，因为_refresh_all处理了大部分重置
        self.aggregator.reset()
        self.digit_mode_active = False
        self.target_digit = None
        self.waiting_for_measurement = False
        ...
        try:
            ...
            if self.waiting_for_measurement:
                if self.aggregator.check_monitoring_complete():
                    ...
                    if result.get('distance_cm') is not None:
                        self.param_vars['距离 (cm)'].set(f"{result['distance_cm']:.2f}")
                    
                    if result.get('size_cm') is not None:
                        self.param_vars['边长/直径 (cm)'].set(f"{result['size_cm']:.2f}")
                    else:
                         self.param_vars['边长/直径 (cm)'].set("未测得")
                    ...
                    if self.aggregator.is_monitoring:
                        ...
                        mode_text = f"数字 {self.target_digit}" if self.digit_mode_active else self.current_mode.get()
    ...
    def on_closing(self):
        """处理窗口关闭事件"""
        if messagebox.askokcancel("退出", "确定要关闭应用程序吗?"):
            print("正在关闭应用程序...")
            self.stop_event.set()
