...
def pnp_distance_mapping(pnp_distance):
    """
    根据实际测量数据对PnP距离进行映射校正
    使用线性插值方法
    """
    # 映射数据：(实际距离, PnP测量距离)
    mapping_data = [
        (100, 70), (110, 76), (120, 83), (130, 89), (140, 95),
        (150, 102), (160, 109), (170, 115), (180, 122), (190, 129), (200, 136)
    ]
    
    # 提取PnP测量值和实际值
    pnp_values = [pair[1] for pair in mapping_data]
    actual_values = [pair[0] for pair in mapping_data]
    
    # 使用numpy进行线性插值
    corrected_distance = np.interp(pnp_distance, pnp_values, actual_values)
...
def test_shape_detection():
    ...
    # 初始化移动平均值滤波器
    avg_filter = MovingAverageFilter(window_size=8)
    ...
    while True:
        try:
            ...
            # 应用PnP距离映射校正
            D_corrected = pnp_distance_mapping(D_raw)
            ...
            if shape:
                ...
                print(f"检测到形状: {shape}")
                print(f"原始值 - 尺寸: {x:.2f}cm, 距离: {D_corrected:.2f}cm (PnP原始: {D_raw:.2f}cm)")
                ...
                # 在图像上绘制信息（显示平均值）
                cv2.putText(result_frame, f"Size: {x_avg:.2f}cm", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
                cv2.putText(result_frame, f"Distance: {D_avg:.2f-0.5}cm", (10, 50), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
                # cv2.putText(result_frame, f"Raw PnP: {D_raw:.2f}cm", (10, 70), 
