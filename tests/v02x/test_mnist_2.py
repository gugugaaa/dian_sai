...
class DigitExtractor:
    ...
    def rotate_image_to_align_short_side(self, image, rect, target_position='bottom'):
        ...
        对于轴对齐的外接矩形：
        - 横向矩形（宽>高）：数字沿短边（高度方向），90°->top, 270°->bottom
        - 纵向矩形（高>宽）：数字沿短边（宽度方向），0°->top, 180°->bottom
        """
        center = rect[0]
        width, height = rect[1]
...
def main():
    ...
    plt.title("3. S1 - Binary Processed (Otsu)")
    plt.axis('off')
    plot_idx += 1
    ...
    plt.title("Recognition Results (Improved Model)")
    plt.axis('off')
