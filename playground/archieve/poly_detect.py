...
def test_line_detection_comprehensive(detector):
    ...
    # 调用内置的测试方法
    detector.test_line_detection()
    
    # 额外的详细测试
    print("\n=== 详细连线检测测试 ===")
    
    # 测试外轮廓内的连线
    if len(detector.outer_polygons) > 0:
        outer_poly = detector.outer_polygons[0]
        vertices = outer_poly['approx']
        
        print(f"\n外轮廓0有 {len(vertices)} 个顶点")
        
        # 测试相邻顶点连线（边缘）
        for i in range(len(vertices)):
            next_i = (i + 1) % len(vertices)
            result = detector.check_line_by_vertex_indices(
                outer1_idx=0, vertex1_idx=i,
                outer2_idx=0, vertex2_idx=next_i
            )
            coord1 = vertices[i][0]
            coord2 = vertices[next_i][0]
            print(f"  边缘连线 顶点{i}-{next_i} ({coord1[0]},{coord1[1]})-({coord2[0]},{coord2[1]}): {'✓在图形内' if result else '✗不在图形内'}")
        ...
        if len(vertices) >= 4:
            ...
            for i in range(len(vertices)):
                for j in range(i + 2, len(vertices)):
                    if j == len(vertices) - 1 and i == 0:  # 跳过最后一个点到第一个点的连线（这是边缘）
                        continue
                    result = detector.check_line_by_vertex_indices(
                        outer1_idx=0, vertex1_idx=i,
                        outer2_idx=0, vertex2_idx=j
                    )
                    coord1 = vertices[i][0]
                    coord2 = vertices[j][0]
                    print(f"  其他 顶点{i}-{j} ({coord1[0]},{coord1[1]})-({coord2[0]},{coord2[1]}): {'✓在图形内' if result else '✗不在图形内'}")
    
    # 测试内轮廓连线
    if len(detector.inner_polygons) > 0:
        print(f"\n内轮廓连线测试:")
        inner_poly = detector.inner_polygons[0]
        vertices = inner_poly['approx']
        
        print(f"内轮廓0有 {len(vertices)} 个顶点")
        
        # 测试内轮廓相邻顶点连线
        for i in range(min(3, len(vertices))):  # 只测试前几个，避免输出太多
            next_i = (i + 1) % len(vertices)
            result = detector.check_line_by_vertex_indices(
                inner1_idx=i, inner2_idx=next_i
            )
            coord1 = vertices[i][0]
            coord2 = vertices[next_i][0]
            print(f"  内轮廓边缘 顶点{i}-{next_i} ({coord1[0]},{coord1[1]})-({coord2[0]},{coord2[1]}): {'✓在图形内' if result else '✗不在图形内'}")
    
    # 测试外轮廓到内轮廓的连线
    if len(detector.outer_polygons) > 0 and len(detector.inner_polygons) > 0:
        print(f"\n外轮廓到内轮廓连线测试:")
        outer_vertices = detector.outer_polygons[0]['approx']
        inner_vertices = detector.inner_polygons[0]['approx']
        
        # 测试几条代表性连线
        for i in range(min(2, len(outer_vertices))):
            for j in range(min(2, len(inner_vertices))):
                point1 = {'contour_type': 'outer', 'contour_id': 0, 'vertex_id': i}
                point2 = {'contour_type': 'inner', 'contour_id': 0, 'vertex_id': j}
                result = detector.is_line_inside_shape(point1, point2)
                
                coord1 = outer_vertices[i][0]
                coord2 = inner_vertices[j][0]
                print(f"  外{i}-内{j} ({coord1[0]},{coord1[1]})-({coord2[0]},{coord2[1]}): {'✓在图形内' if result else '✗不在图形内'}")
    ...
    for p1, p2 in test_lines:
        result = detector.is_line_inside_shape(p1, p2)
        print(f"  坐标连线 {p1}-{p2}: {'✓在图形内' if result else '✗不在图形内'}")
